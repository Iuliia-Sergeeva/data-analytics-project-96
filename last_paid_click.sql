...
with Last_Paid_Click as (
    select
        ...
        case
            when l.created_at < visit_date then null else l.lead_id
            end as lead_id,
        case
            when l.created_at < visit_date then null else l.created_at
            end as created_at,
        l.amount,
        l.closing_reason,
        l.status_id,
        row_number()
            over (partition by l.lead_id order by s.visit_date desc) as rn
from sessions s
left join leads l
